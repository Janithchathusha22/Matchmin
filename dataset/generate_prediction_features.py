...
def main():
    ...
    for m in sorted_matches:
        ...
        def get_rolling_stats(history):
            ...
            def verified_average(key, fallback):
            ...
            return {
                ...
                'possession': verified_average('possession', 50.0),
                'shots': verified_average('shots', 12.0),
                'shots_on_target': verified_average('shots_on_target', 4.0),
                'saves': verified_average('saves', 3.0),
                'corners': verified_average('corners', 5.0),
                'fouls': verified_average('fouls', 12.0),
                'offsides': verified_average('offsides', 2.0),
                'xg_scored': verified_average('xg_scored', 1.3),
                'xg_conceded': verified_average('xg_conceded', 1.3)
            }
        ...
        if is_completed:
            ...
            home_xg = optional_float(m['home_xg'])
            away_xg = optional_float(m['away_xg'])
        ...
        if is_completed:
            h_stats = stats_map.get((mid, h_id), {})
            a_stats = stats_map.get((mid, a_id), {})
            ...
            team_history[h_id].append({
                ...
                'xg_scored': home_xg,
                'xg_conceded': away_xg,
                'possession': h_stats.get('possession'),
                'shots': h_stats.get('shots'),
                'shots_on_target': h_stats.get('shots_on_target'),
                'saves': h_stats.get('saves'),
                'corners': h_stats.get('corners'),
                'fouls': h_stats.get('fouls'),
                'offsides': h_stats.get('offsides')
            })
            ...
            team_history[a_id].append({
                ...
                'xg_scored': away_xg,
                'xg_conceded': home_xg,
                'possession': a_stats.get('possession'),
                'shots': a_stats.get('shots'),
                'shots_on_target': a_stats.get('shots_on_target'),
                'saves': a_stats.get('saves'),
                'corners': a_stats.get('corners'),
                'fouls': a_stats.get('fouls'),
                'offsides': a_stats.get('offsides')
            })
