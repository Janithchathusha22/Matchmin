...
> **Current integrity model:** Live results and prediction-critical contender snapshots are source-linked and versioned. Observed, derived, prediction, and legacy placeholder fields are kept distinct through `data_manifest.json`; missing verified statistics are never silently changed to zero.
...
- **Matches Completed:** 97
- **Teams Remaining:** 7 (FRA, ESP, BEL, NOR, ENG, ARG, SUI)
...
12. **`remaining_teams.csv`**: Verified seven-team contender list, frontend image mapping, and tournament status as of 10 July 2026.
13. **`current_bracket.csv`**: Quarter-final-onward bracket snapshot with observed results separated from unresolved source slots.
14. **`remaining_team_snapshot.csv`**: Derived contender form and verified-stat aggregates generated by `python -m matchmind.snapshot`.
15. **`locked_predictions.json`**: Versioned model forecast snapshot with its data fingerprint; this is explicitly prediction data, not observed fact.
16. **`data_manifest.json`**: Provenance, dataset version, verification links, and observed/derived/prediction classification.
...
- Current prediction-critical files follow `data_manifest.json`: unknown xG, lineup, or event values remain blank instead of being converted to zero.
- The legacy dataset generator contains seeded placeholder fields. Those fields must not be described as verified observations or promoted into the live prediction snapshot without provenance.
